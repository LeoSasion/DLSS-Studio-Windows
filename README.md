...
- 新结果及切换预览时自动等比适应窗口，完整显示横屏和竖屏 4K；保留 100% 原像素、滚轮缩放与键盘/鼠标平移。视频的 100% 指当前预览文件的像素，预览分辨率可能小于下载结果。
...
- 按旋转信息正确处理竖拍视频；默认 MP4，Windows 11 默认 AV1、Windows 10 默认 H.264，逐项检查硬件编码能力，不可用时采用同格式 CPU 编码。
- 推荐码率自动随输出分辨率调整：1080p（1K）6 Mbps、2K（1440p）9 Mbps、4K 12 Mbps；720p 使用 6 Mbps，原始尺寸按素材长边匹配。保留手动编码、恒定画质和自定义码率选项，已保存的自定义设置及历史结果保留。
...
31 项自动化测试覆盖显卡匹配、参数边界、任务取消和恢复、进程树清理、缓存保留、旋转视频尺寸、Windows 默认编码选择、码率映射及 AV1/H.264 实际编码。Windows 工作站另行验证独立解压、包内 Python 依赖加载、EXE 服务启停、端口占用自动切换，以及真实图片增强和短视频 ProRes、默认 AV1 MP4 导出。

独立窗口完成 32 项工作台流程检查，覆盖同步视频对比、时间轴片段范围、播放控制栏、缩放和平移、断连重试、刷新恢复、历史下载、设置复用、收藏保护、6/9/12 Mbps 默认码率及 4K 横竖屏预览；另行验证深浅主题、玻璃持续显示与背景更新、边框不变、最大化/还原、200% 显示缩放及正常退出。实机为 Windows 11、RTX PRO 6000 Blackwell；Windows 10 默认策略已通过模拟测试，30 / 40 系、其他驱动和长视频未全部实测。

原生工作台回归脚本为 `tests/workbench-workflow.js`；测试前将 `tests/fixtures/preview-4k-{landscape,portrait}.mp4` 复制到测试包的 `app/web/test-preview-4k-{landscape,portrait}.mp4`，仅用于回归，不放入发行包。
